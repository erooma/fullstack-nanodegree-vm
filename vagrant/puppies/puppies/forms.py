# forms definition file for application
# AdoptUsDogs - -- implementation of a catalog system
# 23/01/2016 Andrew Moore

from flask_wtf import Form
from flask_wtf.file import FileField, FileAllowed 
from wtforms import BooleanField, StringField, DateTimeField, TextAreaField 
from wtforms import IntegerField, validators, DateField, DecimalField
...
class NewPuppyForm(Form):
    ...
    picture     = FileField('Puppy picture', validators=[FileAllowed(['jpg',\
        'png', 'jpeg', 'gif'], 'Images only!')])
...
class UserForm(Form):
    ...
    level       = IntegerField('User Level', [validators.\
        NumberRange(min=0, max=3, message="The user level is 0\
         (computer), 1(normal), 2(admin), 3(admin+).")])
    # Assigns user admin level:
